...
class SpannedGridLayoutManager : RecyclerView.LayoutManager {
    ...
    private lateinit var cellBorders: IntArray
    ...
    // key == row, val == first child position
    private var firstChildPositionForRow: MutableList<Int>? = null
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int, defStyleRes: Int) {
        ...
        columns = typedArray.getInt(R.styleable.SpannedGridLayoutManager_spanCount, 1)
    ...
    override fun scrollVerticallyBy(
        ...
        if (dy < 0) { // scrolling content down
            ...
            scrolled = if (lastVisiblePosition == itemCount - 1) { // is at end of content
                val scrollRange =
                    (bottom - height + paddingBottom).coerceAtLeast(0)
    ...
    private fun layoutRow(
        ...
        val first =
            cells!![firstPositionInRow]
        val last =
            cells!![lastPositionInRow]
    ...
    private fun calculateWindowSize() {
        // use OrientationHelper#getTotalSpace
        val cellWidth = floor((width - paddingLeft - paddingRight) / columns.toDouble()).toInt()
    ...
    private val minimumFirstVisibleRow: Int
        get() {
            val maxDisplayedRows =
                ceil(height.toFloat() / cellHeight.toDouble()).toInt() + 1
    ...
    private fun measureChildWithDecorationsAndMargin(child: View, widthSpec: Int, heightSpec: Int) {
        ...
        widthSpec = updateSpecWithExtra(
            widthSpec,
            lp.leftMargin + itemDecorationInsets.left,
            lp.rightMargin + itemDecorationInsets.right
        )
        heightSpec = updateSpecWithExtra(
            heightSpec,
            lp.topMargin + itemDecorationInsets.top,
                ...
                if (nominator.isNotEmpty() && denominator.isNotEmpty()) {
                    try {
                        ...
                        if (nominatorValue > 0 && denominatorValue > 0) {
                            cellAspectRatio = abs(nominatorValue / denominatorValue)
                            return
                        }
